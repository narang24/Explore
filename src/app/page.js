...
export default function Home() {
  ...
  const ButtonTwo = ({ children, onClick, className, ...props }) => (
  <button onClick={onClick} className={`rounded-lg font-semibold transition-all ${className}`} {...props}>
    {children}
  </button>
  );
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <ButtonTwo
              onClick={() => setOpenModal(true)}
              radius='md'
              className="bg-white px-8 py-3 text-sm cursor-pointer font-semibold shadow-lg hover:scale-105 transition-all"
            >
              Get Started Now
            </ButtonTwo>
            <ButtonTwo
